...
function parseJSON<T>(raw: string): T {
  // Strip markdown code fences if Groq wraps the response in them
  const cleaned = raw
    .trim()
    .replace(/^```json\s*/i, "")
    .replace(/^```\s*/i, "")
...
export async function generateQuestions(
  ...
  try {
    ...
    // Trim to exact count in case Groq returns more
...
export async function generateSummary(
  ...
  try {
    if (feedbacks.length === 0) {
      throw new Error("No feedback data to summarize.");
    }
